...
def ensure_transaction_schema() -> None:
    ...
    statements = [
        ...
        CREATE TABLE IF NOT EXISTS real_estate.legal_contracts (
            id VARCHAR PRIMARY KEY,
            transaction_session_id VARCHAR
...
def ensure_docusign_schema() -> None:
    ...
    statements = [
        ...
        """,
        """
        CREATE INDEX IF NOT EXISTS ix_legal_contracts_property_id
        ON real_estate.legal_contracts (property_id)
        """,
        """
        CREATE INDEX IF NOT EXISTS ix_legal_contracts_user_id
        ON real_estate.legal_contracts (user_id)
        """,
    ]
    with engine.connect() as connection:
        for statement in statements:
...
def ensure_saved_searches_schema() -> None:
    ...
    Safe to run repeatedly (IF NOT EXISTS + ALTER ADD IF NOT EXISTS for upgrade safety).
    """
    create_statements = [
        """
    ...
    alter_statements = [
        ...
        # Phase 5.0: additive user_id for tenant isolation (no default; all inserts now provide real user_id from auth)
        "ALTER TABLE real_estate.saved_search_alerts ADD COLUMN IF NOT EXISTS user_id VARCHAR",
        ...
        # Phase 5.0: user_id for direct tenant filtering on matches (additive)
        "ALTER TABLE real_estate.saved_search_matches ADD COLUMN IF NOT EXISTS user_id VARCHAR",
